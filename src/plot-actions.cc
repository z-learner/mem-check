...
struct LifeBlock {
  AllocOp start_op{AllocOp::Unknown};
  AllocOp end_op{AllocOp::Unknown};
  size_t start_tid{0};
  size_t end_tid{0};
  ...
  void *start_caller{nullptr};
  void *end_caller{nullptr};
...
std::string hsv_2_rgb(double hue, double sat, double val) {
  int i = (int)(hue * 6);
  double f = hue * 6 - i;
  double p = val * (1 - sat);
...
struct SvgWriter {
  ...
  explicit SvgWriter(std::string path, double width, double height,
                     double margin = 0)
      : out(path), full_width(width), full_height(height), margin(margin),
        is_html(path.rfind(".htm") != std::string::npos) {
    if (is_html) {
      out << "<!DOCTYPE html>\n<html>\n<head>\n";
      out << "<style>\n";
  ...
  std::string def_gradient(std::string const &color1,
  ...
  void rect(double x, double y, double width, double height,
    ...
    x += margin;
    out << "<rect width=\"" << width << "\" height=\"" << height << "\" x=\""
        << x << "\" y=\"" << y << "\" fill=\"" << color << "\"/>\n";
  }
  ...
  ~SvgWriter() {
    ...
    if (is_html) {
      ...
      out << "<script>\nvar svgFullWidth = " << full_width << ";\n";
      out << "var svgFullHeight = " << full_height << ";\n";
      out << "function fit() {\n";
...
PlotOptions parse_plot_options_from_env() {
  PlotOptions options;
  auto env = std::getenv("MEM_CHECK");
  if (!env) {
    return options;
  }
  // MEM_CHECK=format:obj;path:/tmp/malloc.obj;height_scale:log;z_indicates:thread;layout:timeline;show_text:0;text_max_height:24;text_height_fraction:0.4;filter_cpp:1;filter_c:1;filter_cuda:1;svg_margin:420;svg_width:2000;svg_height:1460
  std::string s(env);
...
void mem_check_plot_alloc_actions(std::vector<AllocAction> actions) {
  PlotOptions options = detail::parse_plot_options_from_env();
  ...
  std::map<void *, detail::LifeBlock> living;

  std::set<detail::LifeBlock, detail::LifeBlockCompare> dead;

  for (auto const &action : actions) {
    if (!options.filter_c && kAllocOpIsC[static_cast<size_t>(action.op)]) {
      continue;
    }
    ...
    if (kAllocOpIsAllocation[static_cast<size_t>(action.op)]) {
      living.insert({action.ptr,
                     {action.op, action.op, action.tid, action.tid, action.ptr,
                      action.size, action.caller, action.caller,
      ...
      if (it != living.end()) {
        ...
        it->second.end_time = action.timestamp;
  ...
  double (*eval_height)(detail::LifeBlock const &);
  if (options.height_scale == PlotOptions::Log) {
    eval_height = [](detail::LifeBlock const &block) -> double {
  ...
  if (options.format == PlotOptions::Obj) {
    detail::ObjWriter obj(options.path.empty() ? "malloc.obj" : options.path);
    ...
    auto eval_z =
        [&](detail::LifeBlock const &block) -> std::pair<double, double> {
      double z0 = 0;
      double z1 = 0;
    ...
    auto caller_color = [&](void *caller) -> std::string {
      ...
      return detail::hsv_2_rgb(hue, 0.7, 0.7);
    };

    auto eval_color = [&](detail::LifeBlock const &block)
        -> std::pair<std::string, std::string> {
      return {caller_color(block.start_caller), caller_color(block.end_caller)};
    };

    auto eval_text = [](detail::LifeBlock const &block)
        -> std::pair<std::string, std::string> {
      return {addr2sym(block.start_caller), addr2sym(block.end_caller)};
    };

    detail::SvgWriter svg(options.path.empty() ? "/tmp/mem-check.html"
                                               : options.path,
    ...
    if (options.layout == PlotOptions::Address) {
      for (auto const &block : dead) {
        ...
        auto gradColor = svg.def_gradient(color1, color2);
        svg.rect(x, y, width, height, gradColor);
        if (options.show_text) {
          auto [text1, text2] = eval_text(block);
          auto fontHeight =
              std::min((size_t)(height * options.text_height_fraction + 0.5),
    ...
    auto repeat = [&](int64_t d, char const *s, char const *end) {
      size_t n =
          std::max(d * screen_width, (int64_t)0) / (end_time - start_time + 1);
      std::string r;
